...
useEffect(() => {
  dispatch({
    ...
    payload: {
      ...
      calculatedSalary: props.calculatedSalary,
      presents: props.presents,      // added
      absents: props.absents,        // added
      halfdays: props.halfdays,      // added
      workingdays: props.workingdays,// added
    },
  });
}, [props]);
...
const handleSaveAttendance = async () => {
  ...
  try {
    const safeMonth = new Date(state.month); // ✅ clone to avoid mutation
    ...
    dispatch({
      ...
      payload: {
        ...
        salary: res.finalSalaryToPay,            // note: match reducer field names
        finalSalary: res.finalSalary,
        presents: res.presents,             // <-- add these
        absents: res.absents,
   ...
   return (
    <div className="relative max-w-md w-full">
      ...
      >
        {/* 👤 Employee Details */}
        <div>
          <h2 className="text-xl font-bold mb-2 text-white">👤 Employee Details</h2>
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-400">
                🧾 Previous Balance: ₹{state.balance.toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        {/* 📉 Loan & EMI */}
        <div>
          <h2 className="text-xl font-bold mb-2 text-white">📉 Loan & EMI</h2>
          <p className="text-sm text-gray-400">💳 Loan Remaining: ₹{props.loanRemaining}</p>
        </div>

        {/* 📊 Attendance Summary */}
        <div>
          <h2 className="text-xl font-bold mb-2 text-white">📊 Attendance Summary</h2>
        ...
        {/* 💸 Salaries & Deductions */}
        <div>
          <h2 className="text-xl font-bold mb-2 text-white">💸 Salaries & Deductions</h2>
        ...
        <AddTransaction
          ...
          onAdd={({ transaction, balance, salary, deductions }) =>
            dispatch({
              ...
              payload: { transaction, balance, salary, deductions },
            })
          }
        />
      </div>

      {/* Overlay loading text - sharp, not blurred */}
      {isLoading && (
        <div
          style={{
            position: 'absolute',
